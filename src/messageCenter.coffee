...
class MessageCenter extends EventEmitter
    ...
    denormalize:(obj,option = {})->
        ...
        if obj instanceof Array
            return (@denormalize(item,option) for item in obj)
        else if result = @customDenormalize(obj)
            return result
        else if obj.__mc_type is "buffer"
            return new BufferObject(obj.value,"base64")
        else if obj.__mc_type is "date"
            ...
            return new ReadableStream(option.owner,obj.id)
        else if ClassObject = @getMatchingSerializableType(obj.__mc_type)
    ...
    normalize:(obj)->
        ...
        if obj is null
            return null
        else if result = @customNormalize(obj)
            return result
        else if obj instanceof BufferObject
            return {__mc_type:"buffer",value:obj.toString("base64")}
        else if obj instanceof Date
            ...
            return {__mc_type:"stream",id:obj.id}
        else if ClassObject = @getMatchingSerializableType(obj)
            return {__mc_type:obj.__mc_type,serialized:obj.serialize()}
        else if obj.toJSON
            return obj.toJSON()
        else
            _ = {}
            for prop of obj
                _[prop] = @normalize(obj[prop])
            return _
    getMatchingSerializableType:(obj)->
        if not obj
            return null
        if typeof obj is "string"
            return @customTypes[obj]
        ClassObject = @customTypes[obj.__mc_type]
        if not ClassObject
            return null
        if obj instanceof ClassObject
            return ClassObject
        return null
    registerSerializableType:(name,ClassObject)->
        if ClassObject::__mc_type isnt name
    ...
    handleEvent:(info)->
        ...
        @emit "remoteEvent",info.name,info.params
